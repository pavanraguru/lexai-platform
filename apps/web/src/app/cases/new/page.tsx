...
interface FormData {
  ...
  filed_date: string; fir_number: string; police_station: string;
  complainant_name: string; accused_names: string; sections_charged: string;
  opposing_counsel: string; case_value_inr: string; assigned_advocates: string[];
}
...
export default function NewCasePage() {
  ...
  const createCase = useMutation({
    mutationFn: () => {
      const sections = form.sections_charged ? form.sections_charged.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
      ...
      return casesApi.create(token!, {
        ...
        assigned_advocates: form.assigned_advocates.length > 0 ? form.assigned_advocates : ['placeholder'],
        metadata: {
    ...
    onSuccess: (res: any) => router.push(`/cases/${res.data.id}`),
    onError: (err: any) => setError(err.message || 'Failed to create case'),
  });

  const set = (key: keyof FormData, val: any) => setForm((f: FormData) => ({ ...f, [key]: val }));
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center gap-2 flex-1">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${i <= step ? 'text-white' : 'bg-gray-100 text-gray-400'}`}
              style={i <= step ? { backgroundColor: '#1E3A5F' } : {}}>
              {i < step ? <Check size={14} /> : i + 1}
            </div>
            <span className={`text-xs hidden sm:block ${i === step ? 'font-semibold text-gray-900' : 'text-gray-400'}`}>{s}</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        ...
        {step === 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {CASE_TYPES.map(ct => (
                <button key={ct.value} onClick={() => set('case_type', ct.value)}
                  className={`flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all ${form.case_type === ct.value ? 'border-[#1E3A5F] bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                  <span className="text-xl">{ct.icon}</span>
                  <div>
                    <p className={`text-sm font-semibold ${form.case_type === ct.value ? 'text-[#1E3A5F]' : 'text-gray-800'}`}>{ct.label}</p>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <div>
              <h2 className="text-lg font-bold text-gray-900 mb-4">Which court?</h2>
              <select value={form.court}
                ...
                {courtList.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
              </select>
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {PERSPECTIVES.map(p => (
                  <button key={p.value} onClick={() => set('perspective', p.value)}
                    className={`p-3 rounded-xl border-2 text-left transition-all ${form.perspective === p.value ? 'border-[#1E3A5F] bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <p className={`text-sm font-semibold ${form.perspective === p.value ? 'text-[#1E3A5F]' : 'text-gray-800'}`}>{p.label}</p>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Case title <span className="text-red-500">*</span></label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={form.complainant_name}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('complainant_name', e.target.value)}
                  ...
                  <div>
                    ...
                    <input type="text" value={form.fir_number}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('fir_number', e.target.value)}
                      placeholder="FIR No. 456/2024"
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Sections charged</label>
                  <input type="text" value={form.sections_charged}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('sections_charged', e.target.value)}
            ...
            <div>
              ...
              <input type="text" value={form.opposing_counsel}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('opposing_counsel', e.target.value)}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={form.cnr_number}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('cnr_number', e.target.value)}
                  placeholder="DLHC01-001234-2024"
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none text-sm font-mono" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Priority</label>
                <select value={form.priority}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => set('priority', e.target.value)}
              ...
              <div>
                ...
                <input type="date" value={form.filed_date}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('filed_date', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Judge Name</label>
                <input type="text" value={form.judge_name}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('judge_name', e.target.value)}
      ...
      <div className="flex justify-between mt-6">
        <button onClick={() => step > 0 ? setStep(s => s - 1) : router.push('/cases')}
